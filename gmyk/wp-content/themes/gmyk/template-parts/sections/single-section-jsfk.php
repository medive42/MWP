            ...
            <?php $posts = get_posts( "category=44&numberposts=5" ); ?>  
             <?php if( $posts ) : ?>  
              <ul><?php foreach( $posts as $post ) : setup_postdata( $post ); ?>  
                  <li style="height:44px;padding-top:10px;font-size:14px;">  
...
<div class="fr w293" >	  
 ...
 <!-- 获取指定栏目文章列表-->  
      <div class="sideList">
           <ul>
            <?php $posts = get_posts( "category=107&numberposts=6" ); ?>  
             <?php if( $posts ) : ?>  
              <ul><?php foreach( $posts as $post ) : setup_postdata( $post ); ?>  
                  <li style="height:44px;padding-top:10px;font-size:18px;">  
                      <a style="font-size:14px;padding-left:20px;" href="<?php the_permalink() ?>" rel=”bookmark” title=”<?php the_title(); ?>”>* <?php the_title(); ?></a>  
                  </li>  
                  <?php endforeach; ?>  
              </ul>  
             <?php endif; ?>
